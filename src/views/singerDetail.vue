...
export default {
  ...
  methods: {
    _getSingerDetail () {
      ...
      // 如果客户端在歌手详情页面进行了刷新 vuex 保存的歌手数据就会被初始化，所以加载不到数据，那么我们就让他返回的歌手列表页
      if (!mid) {
      ...
      getSingerDetail(mid).then(res => {
        if (res.data.code === ERR_OK) {
          const data = this._initDetailData(res.data.data.list)
    ...
    _initDetailData (data) {
      ...
      const arr = data.map(item => {
        const { musicData: { albummid, albumname, songmid, songname, singer, songid } } = item
        return {
          albummid,
          songmid,
          songname,
  ...
  computed: {
    ...
    backImage () {
      return baseImageURL(this.singer.singer_mid, 1)
    },
    ...mapGetters([
      'singer'
    ])
